...
if __name__ == '__main__':
    GetTestCase.case_data(['login_01'])


    # a = CaseData(r'D:\TestFrame\PytestAutoApi-master\data\Collect\collect_addtool.yaml').case_process()
    # print(a)
    #
    # dates = GetYamlData(r'D:\TestFrame\PytestAutoApi-master\data\Collect\collect_addtool.yaml').get_yaml_data()
    # case_id_switch = True
    # case_lists = []
    # a = CaseData(r'D:\TestFrame\PytestAutoApi-master\data\Collect\collect_addtool.yaml')
    # for key, values in dates.items():
    #     # 公共配置中的数据，与用例数据不同，需要单独处理
    #     if key != 'case_common':
    #         case_date = {
    #             'method': a.get_case_method(case_id=key, case_data=values),
    #             'is_run': a.get_is_run(key, values),
    #             'url': a.get_case_host(case_id=key, case_data=values),
    #             'detail': a.get_case_detail(case_id=key, case_data=values),
    #             'headers': a.get_headers(case_id=key, case_data=values),
    #             'requestType': a.get_request_type(key, values),
    #             'data': a.get_case_dates(key, values),
    #             'dependence_case': a.get_dependence_case(key, values),
    #             'dependence_case_data': a.get_dependence_case_data(key, values),
    #             "current_request_set_cache": a.get_current_request_set_cache(values),
    #             "sql": a.get_sql(key, values),
    #             "assert_data": a.get_assert(key, values),
    #             "setup_sql": a.setup_sql(values),
    #             "teardown": a.tear_down(values),
    #             "teardown_sql": a.teardown_sql(values),
    #             "sleep": a.time_sleep(values),
    #         }
    #
    #         if case_id_switch is True:
    #             case_lists.append({key: TestCase(**case_date).dict()})
