...
class TestCase(BaseModel):
    ...
    dependence_case: Union[None, bool] = False
    dependence_case_data: Optional[Union[None, List["DependentCaseData"], Text]] = None
    sql: Optional[List[Any]] = None
    setup_sql: Optional[List[Any]] = None
    status_code: Optional[int] = None
    teardown_sql: Optional[Any] = []
    # teardown: Union[List["TearDown"], None] = []
    teardown: Optional[List["TearDown"]] = []  # 设置默认值为空列表
    current_request_set_cache: Optional[List["CurrentRequestSetCache"]] = []
...
class ResponseData(BaseModel):
    ...
    status_code: int
    teardown: Optional[List["TearDown"]] = []
