...
class SendEmail:
    ...
    def send_mail(cls, user_list: list, sub, content: str) -> None:
        ...
        @return:
        """
        user = formataddr(("吴庭威", config.email.send_user))
        ...
        try:
            # 连接SMTP服务器
            server = smtplib.SMTP(config.email.email_host, 587)
            server.starttls()  # 启用TLS加密，推荐使用
            server.login(config.email.send_user, config.email.stamp_key)
            server.sendmail(config.email.send_user, user_list, message.as_string())
